<div class="animate-zoom-in" x-data="quoter">
    ...
    <div class="flex">
        <div class="relative flex justify-center w-full mt-2" id="container-quoter" x-show="active"
            ...
            <div class="bg-white border border-gray-200 rounded-lg shadow w-96 dark:bg-gray-800 dark:border-gray-700">
                <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 rounded-t-lg select-none justify-evenly bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800"
                    role="tablist">
                    <li class="me-2">
                        <button type="button" @click="purchase = true" role="tab"
                            id="tab-purchase"
                    ...
                    <li class="me-2">
                        <button type="button" @click="purchase = false" role="tab"
                            id="tab-sales"
        ...
        <div class="absolute flex justify-center w-full mt-2 -z-10" id="container-promo"
            x-show="!active"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-90"
            x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-300"
            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90">
                ...
                <div class="flex flex-col items-center justify-around p-4 h-96">
                    ...
                    <div>
                        <div class="text-base text-gray-900 text-pretty">
                            Los <b>ALSERPROMOS</b> te permiten acceder a un tipo de cambio insuperable, específicos y
                            por monto mínimo.
                        </div>
            ...
            Alpine.data('quoter', () => ({
                ...
                init() {
                    ...
                    this.$watch('purchase', () => {
                        ...
                        document.getElementById('tab-purchase').classList.toggle('text-home-primary');
                        document.getElementById('tab-purchase').classList.toggle('border-b');
                        document.getElementById('tab-purchase').classList.toggle('border-indigo-700');
                        document.getElementById('tab-purchase').classList.toggle('border-gray-600');
                        document.getElementById('tab-sales').classList.toggle('text-home-primary');
                        document.getElementById('tab-sales').classList.toggle('border-b');
                        document.getElementById('tab-sales').classList.toggle('border-indigo-700');
                        document.getElementById('tab-sales').classList.toggle('border-gray-600');
                    });
                    this.$watch('active', () => {
                        document.getElementById('toggle-quoter').classList.toggle('bg-home-primary');
                        document.getElementById('toggle-quoter').classList.toggle('text-white');
                        document.getElementById('toggle-quoter').classList.toggle('tex-gray-700');
                        document.getElementById('toggle-promo').classList.toggle('bg-home-primary');
                        document.getElementById('toggle-promo').classList.toggle('text-white');
                        document.getElementById('toggle-promo').classList.toggle('tex-gray-700');
                        document.getElementById('container-quoter').classList.toggle('absolute');
                        document.getElementById('container-quoter').classList.toggle('-z-10');
                        document.getElementById('container-quoter').classList.toggle('relative');
